...
- El "nuevo ángulo" es obligatorio: explicá el concepto central del módulo desde una perspectiva distinta a la del módulo base, con un ejemplo cotidiano concreto y nuevo.
- La práctica puede ser la misma si es fundamental al módulo, o una variante — pero siempre adaptada al estado del check-in del día.
...
- La pregunta de cierre es siempre nueva — nunca la misma que el módulo original.
- Extensión total: 2-3 min de lectura. Concentrado, no denso.
...
ESTRUCTURA DE SALIDA — RESPONDER SOLO EN JSON VÁLIDO, SIN BACKTICKS NI TEXTO ADICIONAL:
{
  ...
  "nuevo_angulo": {
    ...
    "texto": "el concepto central del módulo explicado desde un ángulo nuevo, con ejemplo cotidiano concreto. máx 120 palabras. Nunca repetir ejemplos del módulo original.",
    "idea_clave": "la idea más importante de este nuevo ángulo, en 1 frase. Sustancia, sin adornos."
  },
  "practica": {
    "titulo": "nombre de la práctica, máx 6 palabras",
    "es_variante": true or false,
    "nota_variante": "si es_variante es true: en 1 línea, qué cambia respecto a la práctica original. Si es false: null.",
    "duracion": "duración concreta adaptada al tiempo disponible y al estado del check-in",
    "pasos": ["paso 1 concreto y accionable", "paso 2", "paso 3"],
    "cierre": "pregunta de autoconocimiento genuina al finalizar, nueva, máx 20 palabras"
  }
}`;
}
...
// ─── HOME VIEW ────────────────────────────────────────────────
function HomeView({ user, onSelectModule, onShowPillar, onAbrirMotor }) {
...
// ─── HOME VIEW ────────────────────────────────────────────────
function HomeView({ user, onSelectModule, onShowPillar, onAbrirMotor, onDevMotor }) {
  const pillar = getPillar(user.currentModule);
  const mod = MODULES[user.currentModule];
  const totalDone = user.completedModules.length;
  const WEEK_ROLES = [{day:"Lunes",role:"Lectura del módulo",icon:"📖"},{day:"Mar–Jue",role:"Práctica diaria",icon:"🎯"},{day:"Viernes",role:"Reflexión y diario",icon:"✏️"},{day:"Sáb–Dom",role:"Integración libre",icon:"🌿"}];
  ...
  return (
    <div className="flex-1 overflow-y-auto pb-24">
      ...
      {/* DEV MODE — botón acceso rápido al motor IA */}
      <div className="px-6 mb-4">
        <button onClick={onDevMotor}
          className="w-full py-2.5 rounded-xl text-xs font-bold"
          style={{background:"rgba(255,180,0,0.08)",border:"1px dashed rgba(255,180,0,0.3)",color:"rgba(255,180,0,0.7)",cursor:"pointer",letterSpacing:"1px"}}>
          ⚡ DEV — Ir a Hapi (simula módulo 18 completado)
        </button>
      </div>
...
export default function HapiApp() {
  // ── Cargar estado guardado ────────────────────────────────────
  // DEV MODE: siempre desde onboarding
